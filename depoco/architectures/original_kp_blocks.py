...
class KPConv(nn.Module):
    ...
    def init_KP(self):
        ...
        K_points_numpy = getKernelPoints(self.radius,
                                      self.K)
    ...
    def forward(self, q_pts, s_pts, neighb_inds, x):
        ...
        # if sq_distances.grad is not None:
        #     print(sq_distances.grad.max())
        # print(sq_distances.requires_grad)
        # print(sq_distances.dtype)
        ...
        # print('KP extend',self.KP_extent,'KP radius',self.radius)
...
#################### SampleNet ####################################################
class SampleResnetBottleneckBlock(nn.Module):

    def __init__(self, config):
        """
        Initialize a resnet bottleneck block.
        :param in_fdim: dimension input features
        :param out_fdim: dimension input features
        :param radius: current radius of convolution
        :param config: parameters
        """
        super(SampleResnetBottleneckBlock, self).__init__()
...
import numpy as np
def getKernelPoints(radius, num_kernel_points):
    if round(num_kernel_points**(1/3)) % 1 == 0:
        npoints = round(num_kernel_points**(1/3))
        xyz = np.linspace(-1, 1, npoints)
        points = np.meshgrid(xyz, xyz, xyz)
